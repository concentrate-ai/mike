/**
 * Concentrate AI provider — universal model router.
 *
 * Concentrate exposes an OpenAI-Responses-compatible endpoint that proxies
 * to many underlying model authors (Anthropic, Google, OpenAI, Meta, …)
 * behind a single API key. This adapter is structurally the same as
 * backend/src/lib/llm/openai.ts (same Responses API, same SSE event stream)
 * but with a different base URL and auth key.
 *
 * The base URL can be overridden via CONCENTRATE_RESPONSES_URL for staging
...
const DEFAULT_RESPONSES_URL = "https://api.concentrate.ai/v1/responses";
const MAX_OUTPUT_TOKENS = 16384;

function responsesUrl(): string {
    return process.env.CONCENTRATE_RESPONSES_URL?.trim() || DEFAULT_RESPONSES_URL;
}
...
function apiKey(override?: string | null): string {
    const key = override?.trim() || process.env.CONCENTRATE_API_KEY?.trim() || "";
    if (!key) {
        throw new Error(
            "Concentrate API key is not configured. Set CONCENTRATE_API_KEY or add a user Concentrate key.",
        );
    }
    return key;
}
...
async function createResponse(params: {
    ...
    if (!response.ok) {
        ...
        const err = new Error(
            `Concentrate request failed (${response.status}): ${text || response.statusText}`,
        );
...
export async function streamConcentrate(
    ...
    for (let iter = 0; iter < maxIter; iter++) {
        ...
        if (!response.body) throw new Error("Concentrate response had no body");
        ...
        // Preserve the conversation: the assistant's function_call items must
        // travel back alongside the function_call_output items, otherwise the
        // Responses API loses track of which call each output answers.
        // Clear previousResponseId so the next request carries the full input
        // inline rather than relying on a stored response that lacks the
...
export async function completeConcentrateText(params: {
    model: string;
    systemPrompt?: string;
    user: string;
    maxTokens?: number;
    apiKeys?: { concentrate?: string | null };
}): Promise<string> {
    const response = await createResponse({
        model: params.model,
        ...
        apiKey: apiKey(params.apiKeys?.concentrate),
    });
